...
namespace MichellesWebsite.Controllers
{
    public class ProductController : Controller
    {
        ...
        {
            List<ProductPrice> productPrices = db.ProductPrices.Where(x => x.ID == productID).Where(x => x.dateTo == DateTime.MinValue).ToList();
            if (productPrices.Count == 1)
            {
                productPrices[0].dateTo = DateTime.Now;
            }
            ProductPrice newProductPrice = new ProductPrice() { price = price, dateFrom = DateTime.Now, dateTo = DateTime.MinValue, productID = productID };
